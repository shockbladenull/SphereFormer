...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    if args.data_name == 'nuscenes':
        train_data = RobotCar(args.data_root,
    ...
    if args.data_name == 'nuscenes':
        val_data = RobotCar(data_path=args.data_root,
            voxel_size=args.voxel_size, 
            split='test',
...
def test(val_loader, model, criterion):
    ...
    for i, batch_data in enumerate(val_loader):
        ...
        (coord, xyz, feat, target, offset, inds_reconstruct, file_name) = batch_data
        print("file name :", file_name[0], type(file_name[0]))
        save_file = "/".join(file_name[0].split("/")[:-2]) + "/sphere_velodyne_left_feature32/" + "".join(file_name[0].split("/")[-1])
        ...
        # print(sinput.indices[:20, :])
        # print(sinput.spatial_shape)

        assert batch.shape[0] == feat.shape[0]

        with torch.no_grad():
            output, output_32 = model(sinput, xyz, batch)

            output = output[inds_reconstruct, :]
            output_32 = output_32[inds_reconstruct, :]

            xyz_origin = xyz[inds_reconstruct, :]
        ...
        save_class = output.view(-1, 1)

        save_result = torch.cat((xyz_origin, output_32, save_class), dim=1).cpu().numpy()
        print(save_result.shape)
        save_result = save_result.astype(np.float32)
        save_result.tofile(save_file)
